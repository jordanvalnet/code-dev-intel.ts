...
- The workspace graph `impactedFiles` builds is cached per workspace root for the life of the process, and `dependencyGraph` shares its per-file parse cache. Each call re-walks the tree and re-parses only files whose `(mtime, size)` moved, plus any file written within 2 s of being cached (filesystem timestamps are too coarse to trust that recently). Anything that changes the file set — including assets — or the content of a `tsconfig.json` / `jsconfig.json` / `package.json` redoes every resolution, since that is what decides where a specifier points. Bounded at 50,000 tracked files across 16 workspace roots, least-recently-used evicted first.
