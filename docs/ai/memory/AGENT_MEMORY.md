...
- Evidence: `pnpm --dir "E:\\dev\\github\\code-dev-intel" run test:all` passed (`4` test files, `10` tests).
...
- Evidence: `pnpm --dir "E:\\dev\\github\\code-dev-intel" run test:all` passed (`5` test files, `14` tests).
...
- Evidence: `pnpm --dir "e:\\dev\\github\\code-dev-intel" run test:all` passed (`eslint` + `tsc --noEmit` + `vitest`, 6 test files / 17 tests).
...
- Evidence: `pnpm --dir "e:\\dev\\github\\code-dev-intel" run test:all` passed (`eslint` + `tsc --noEmit` + `vitest`, 7 test files / 20 tests).
...
- Evidence: `pnpm --dir "e:\\dev\\github\\code-dev-intel" run test:all` passed (`eslint` + `tsc --noEmit` + `vitest`, 8 test files / 23 tests).
...
- Evidence: `pnpm --dir "e:\\dev\\github\\code-dev-intel" run test:all` passed (`8` test files / `23` tests), and `pnpm --dir "e:\\dev\\github\\code-dev-intel" run indexer:smoke` returned `{ "ok": true, ... }`.
...
- Evidence: `pnpm --dir "e:\\dev\\github\\code-dev-intel" run test:all` passed; local perf benchmark command added as `pnpm perf:benchmark` and CI mode as `pnpm perf:benchmark:ci`.
...
- Evidence: `node ./scripts/perf-benchmark.mjs --mode=local --iterations=1` returned `{ "ok": true, ... }`; `pnpm --dir "e:\\dev\\github\\code-dev-intel" run test:all` passed (`8` test files / `23` tests).
...
- Evidence: `pnpm lint` clean; `pnpm type-check` clean; `pnpm test` 20 files / 162 tests passed; `pnpm build` ok; `pnpm audit` "No known vulnerabilities found"; release smoke from a packed tarball ok (ensure/status/searchStruct); live run of the built server against E:/dev/github/a private consumer codebase (4,192 scanned files): 23/23 checks — health, tools/describe, getFileOutline, findSymbol, findImplementations (5), findReferences (19 refs / 9 files), findDefinitions, getSymbolContent, findCallers (4), findCallees, dependencyGraph (3 deps / 8 edges), impactedFiles (11 files, 1.3 s), searchText (ripgrep, 219 ms), searchStruct ts+tsx (5 / 293 matches, ast-grep 0.45.3), findDuplicates (3 groups, 3.4 s), both sandbox rejections (400), MCP over HTTP (initialize/tools/list/tools/call) and MCP over stdio.
- Risks: Both graph engines resolve relative imports only, so on alias-heavy codebases (a private consumer codebase uses `@/`) dependencyGraph/impactedFiles stay shallow — tsconfig `paths` support is the next functional gap. The Origin check could reject an HTTP MCP client that sends a non-local Origin; CODE_INTEL_ALLOWED_ORIGINS (or `*`) is the escape hatch. Node 18/20 are EOL; engines still says >=18.
- Next: bump the exact `code-dev-intel.ts` pin in a private consumer codebase to 0.4.0; consider tsconfig paths alias resolution (T-018) and the still-open T-015 hardening items.
- Blockers: none

## [2026-09-04T17:30:00Z] ClaudeOpus | T-018
- Status: done
- Summary: Both module-graph engines now resolve tsconfig/jsconfig `paths` and `baseUrl` aliases, not only relative specifiers — the functional gap T-017 flagged. `dependencyGraph` (services/code-intel-mcp) and `impactedFiles`/`buildWorkspaceGraph` (services/indexer) share a new resolver, `services/indexer/src/import-resolver.ts`, so `@/domain/ports/Port` becomes an internal edge instead of an external dependency and the two tools stop returning near-empty graphs on alias-heavy codebases such as a private consumer codebase. Released as 0.5.0.
