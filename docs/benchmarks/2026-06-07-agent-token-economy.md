...
- **No false positives.** On the ambiguous-type task, the grep-only control had to write out an explicit exclusion list ("Notable false matches I excluded: `PropertyType`, `PropertyStatus`, `PropertyWithImage`, …") and read 14 files to disambiguate. The treatment agent got a type-checker-authoritative result from `findReferences` in one step.
